#!/usr/bin/env python3
"""
Fetch 7747 (CSOP Samsung Electronics Daily (2x) Leveraged Product, HKEX:07747)
daily Asset Under Management (AUM), NAV, units outstanding from HKEXnews
...
locate the 7747 column and read NAV / units / AUM / premium.

NOTE: 7747 shares the same issuer (CSOP) and the same daily consolidated
XLSX as 7709, so we reuse the identical STOCK_ID and only switch the
target column string "7709" -> "7747".

Usage:
  python fetch_7747_aum.py                 # incremental: only missing dates
  python fetch_7747_aum.py --force         # re-download & re-parse everything
  python fetch_7747_aum.py --test          # dry-run on 3 sampled dates
  python fetch_7747_aum.py --history PATH  # custom output json path
"""
import json
import os
import re
...
DEFAULT_HISTORY = os.path.join(HERE, "..", "data", "7747_aum_history.json")
CACHE_DIR = os.path.join(HERE, "..", "data", "cache_7747")
...
def parse_xlsx(path):
    """Locate 7747 column in the horizontal L&I table and extract metrics."""
    wb = load_workbook(path, data_only=True)
    ws = wb[wb.sheetnames[0]]
    grid = [list(row) for row in ws.iter_rows(values_only=True)]

    # Stock Code row -> column index of "7747"
    code_col = None
    for row in grid:
    ...
    #   <currency> | <code == value>
    val_col = code_col          # 7747 code & value share the same column
    ...
    for l in to_fetch:
        ...
        try:
            ...
            if parsed:
                ...
                print(f"  SKIP {l['date']} (7747 column not found)")
